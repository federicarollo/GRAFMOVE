...
class App:
    ...
    def connect_amenity(self):
        """Connect the OSMWayNode of the POI to the nearest Node in the graph (FootNode and BikeNode)."""
        with self.driver.session() as session:
    ...
    def _connect_amenity(tx):
        result = tx.run("""
                        ...
                        MATCH (n:FootNode)
                            WHERE distance(n.location, poi) < 10
                            AND n <> p
                        MERGE (p)-[r:NEAR]->(n)
        ...
        #                 MATCH (p:OSMWayNode)
        #                 WITH p, p.location AS poi
        #                 MATCH (n:BikeNode)  // Modifica questa parte per usare BikeNode invece di RoadJunction
        #                     WHERE distance(n.location, poi) < 10
        #                     AND n <> p
        #                 MERGE (p)-[r:NEAR]->(n)
        #                     ON CREATE SET r.distance = distance(n.location, p.location), r.status = 'active'
    ...
    def _set_location(tx):
        result = tx.run(
        """MATCH (n:OSMWayNode) SET n.location = point({latitude: tofloat(n.lat), longitude: tofloat(n.lon)})""")
        return result.values()
